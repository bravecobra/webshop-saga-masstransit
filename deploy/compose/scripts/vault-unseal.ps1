...
Function UnsealVault () {
    ...
    $key1 = ((Get-Content ./deploy/compose/_data/keys.txt | Select-String -Pattern 'Key 1:') -split ":\s+")[1].Replace("[0m","")
    docker-compose exec vault vault operator unseal -address="$global:VAULT_ADDR" $key1
    $key2 = ((Get-Content ./deploy/compose/_data/keys.txt | Select-String -Pattern 'Key 2:') -split ":\s+")[1].Replace("[0m","")
    docker-compose exec vault vault operator unseal -address="$global:VAULT_ADDR" $key2
    $key3 = ((Get-Content ./deploy/compose/_data/keys.txt | Select-String -Pattern 'Key 3:') -split ":\s+")[1].Replace("[0m","")
